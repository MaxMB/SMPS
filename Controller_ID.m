...
% Simulation
var_v = 1e-8;%(0.01)^2; % measurement noise variance
var_xd = 1e-8;%var_v; % process noise variance
qtz_in = 5 / 2^16; % measurement quantization interval
...
subplot(211), stairs(0:Ts:tf,d,'k'), grid on;
    title(['Forward Converter Ideal - Vi=' num2str(Vi)...
        'V | Rload=' num2str(Rl) 'Ohm']);
